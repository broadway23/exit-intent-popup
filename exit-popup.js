(function() {
   ...
   * Adjust z-index values and other styling here if needed.
   */
  const popupStyles = `
@keyframes pulse {
 0% { transform: scale(1); }
...
#exit-modal {
  ...
  align-items: center;
  z-index: 999999; /* raised to sit on top */
}
#exit-modal-content {
  ...
  z-index: 1000000; /* ensure overlay is on top */
}
#close-modal {
  position: absolute;
  top: 10px;
...
.countdown-label {
  ...
  /**
   * Inject the styles into the document head if they haven't been added.
   */
   ...
   * Returns an object containing references to key elements.
   */
  ...
  function init() {
    ...
    // State variables for exit intent
    ...
    // Improved desktop exit intent detection
    if (!isMobile()) {
    ...
    // Inactivity timer
    let inactivityTimer;
    const inactivityTimeMs = 30000; // 30 seconds
    ...
    // -------------------------------------------------------------------------
    // Create countdown structure in the modal content (before spin text)
    ...
    // Insert countdown before modal text
